...
function call_gemini(string $prompt): string
{
    ...
    while ($attempt < $max_retries) {
        ...
        // Treat curl errors as HTTP 0 for retry evaluation
        if ($err) {
            $http_code = 0;
        }
        ...
        // Retry on 429 Too Many Requests, 5xx server errors, or curl timeout (0)
        if (in_array($http_code, [429, 500, 502, 503, 504]) || $http_code === 0) {
            $attempt++;
            if ($attempt >= $max_retries) {
                break; // Break the loop and fallback to final error
            }
            sleep($retry_delay);
            $retry_delay *= 2; // Exponential backoff: 2s -> 4s
            continue;
        } else {
            // Other errors (e.g. 400 Bad Request, 401 Unauthorized, 403 Forbidden) shouldn't be retried
